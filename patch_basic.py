...
def patch_all_basics():
    ...
    # 先頭2引数(パス)だけ long-path 化し、残りは *args/**kwargs で透過。
    # hf_hub 0.34.4 は etag 等の新引数を渡すため、固定シグネチャだと TypeError になる。
    # *args/**kwargs にして前方互換にすることで diffusers の from_pretrained(Z-Image)が動く。
