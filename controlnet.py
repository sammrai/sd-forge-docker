    ...
    def get_input_data(self, p, unit, preprocessor, h, w):
        ...
        if unit.input_mode == external_code.InputMode.MERGE:
            ...
            if unit.use_preview_as_input and unit.generated_image is not None:
                ...
                resize_mode = external_code.resize_mode_from_value(getattr(p, 'resize_mode', 0))
                if a1111_i2i_image is not None:
                    image = HWC3(np.asarray(a1111_i2i_image))
                    using_a1111_data = True
                else:
                    # txt2img mode: no init image (e.g. Tile ControlNet for HiRes Fix)
                    # create a blank placeholder; actual control comes from latent in process_before_every_sampling
