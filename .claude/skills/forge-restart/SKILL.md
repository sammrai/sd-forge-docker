---
name: forge-restart
description: Restart, rebuild, or deploy the Forge (sd-ui) and comfy containers safely without killing an in-flight generation. Use whenever the user asks to 再起動 / restart / reload / 適用 / デプロイ / 反映 / 入れ替え / 作り直し / 有効化 anything about Forge, sdui, comfy, or sd-forge-docker, whenever they say 生成が終わったタイミングで / アイドルになったら / 空いたら / 落ち着いたら apply something, whenever you are about to run docker compose up -d / restart / build / pull on this stack, when a change to the comfy-router extension, a comfy custom node, a model file, or an ADetailer patch needs to take effect, or when edited code does not seem to be running. Covers deciding between restart and rebuild (the router is baked into the image, so restart alone does not pick it up), catching the idle gap correctly (it is seconds long -- poll state.job at 0.2s intervals, not log silence), building before you catch it, local build overrides, waiting for the API to really be up, and verifying what the running process actually loaded.
---
